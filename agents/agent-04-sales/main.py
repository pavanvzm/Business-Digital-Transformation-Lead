#!/usr/bin/env python3
"""CLI entry point for Agent-04 (Sales & Distribution Agent).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Manufacturing MAS - Sales & Distribution Agent (Agent-04)",
    )
    parser.add_argument(
        "--mode",
...
async def run_serve_mode() -> None:
    agent = SalesAgent()
    await agent.start()
    logger.info("Sales Agent running in serve mode")
    try:
        await agent.run()
...
async def run_validate_mode() -> None:
    print(f"\n{'='*60}")
    print(f"CONFIGURATION VALIDATION — Agent-04 Sales & Distribution")
    print(f"{'='*60}")
    ...
    print(f"\nSales Settings:")
    print(f"  Pricing change HITL threshold: {settings.sales.pricing_change_hitl_pct}%")
    print(f"  Bulk order minimum: ${settings.sales.bulk_order_minimum_usd:,.0f}")
    print(f"  Allocation HITL threshold: {settings.sales.allocation_hitl_threshold_pct}%")
    print(f"\nHITL:")
...
async def main() -> None:
    ...
    logger.info("Sales Agent CLI", mode=args.mode)
